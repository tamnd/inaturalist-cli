...
describes what users get. Pick a format with `-o`, or let inaturalist choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
inaturalist <command> -o table   # aligned columns for reading
inaturalist <command> -o jsonl   # one JSON object per line, for piping
inaturalist <command> -o json    # a single JSON array
inaturalist <command> -o csv     # spreadsheet friendly
inaturalist <command> -o tsv     # tab-separated
inaturalist <command> -o url     # just the URL column
inaturalist <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
inaturalist <command> --fields id,title,url
```
...
```bash
inaturalist <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
inaturalist <command>            # a table, because this is a terminal
inaturalist <command> | wc -l    # JSONL, because this is a pipe
```
